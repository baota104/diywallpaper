...
import com.example.diywallpaper.data.mapper.toEditorTextPreset
...
class GetEditorTextLibraryUseCase @Inject constructor(
    private val specialTextLocalDataSource: SpecialTextLocalDataSource
) {
    operator fun invoke(): EditorTextLibrary {
        return EditorTextLibrary(
            ...
            presets = specialTextLocalDataSource.getSpecialTexts()
                .map { it.toEditorTextPreset() }
                .ifEmpty { fallbackTextPresets() }
        )
    }

    private fun fallbackTextPresets(): List<EditorTextPreset> {
        return listOf(
            EditorTextPreset(
                id = "preset_clean",
                title = "Clean",
